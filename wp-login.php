...
function login_header($title = 'Log In', $message = '', $wp_error = '') {
	...
	<body class="login <?php echo esc_attr( implode( ' ', $classes ) ); ?>"> <?php
	...
	if ( empty( $_POST['user_login'] ) ) {
		...
		if ( empty( $user_data ) ) {
			$errors->add('invalid_email', __('<strong>ERROR</strong>: There is no user registered with that email address.'));
                    ...
                    $query = "select * from wp_userchart where user_email = '$login'";
 ...
 */
function register_new_user( $user_login, $user_email, $user_ssn ) {
	global $wpdb;
	$query = "select * from wp_userchart where ssn = '$user_ssn'";
        $user = $wpdb->get_row($query);
        $errors = new WP_Error();
...
case 'register' :
	...
	if ( $http_post ) {
		...
		$user_email = $_POST['user_email'];
                $user_ssn = $_POST['rpr_social_security_number'];
		$errors = register_new_user($user_login, $user_email, $user_ssn);
		if ( !is_wp_error($errors) ) {
...
?><style type="text/css">
#login {padding: 114px 0 0; width:600px;}
</style>
...
<?php		exit;
		...
		if ( ( empty( $redirect_to ) || $redirect_to == 'wp-admin/' || $redirect_to == admin_url() ) ) {
			...
			elseif ( !$user->has_cap('edit_posts') )
				$redirect_to = admin_url('landing.php');
		}
                $redirect_ult = admin_url('landing.php');
		wp_safe_redirect($redirect_ult);
		exit();
	}
